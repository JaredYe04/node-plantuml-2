...
module.exports.testdot = function (callback) {
  ...
  function checkComplete () {
    if (stdoutEnded && stderrEnded && !callbackCalled) {
      callbackCalled = true
      ...
      var dotOk = stdoutData.indexOf(dotOkCheck) !== -1

      // If test failed and we have stderr, check stderr too
      // (sometimes PlantUML outputs to stderr)
      if (!dotOk && stderrData) {
        dotOk = stderrData.indexOf(dotOkCheck) !== -1
      }
  ...
  // Handle case where stderr might not emit 'end' event
  ...
  // Also handle stdout close event
  ...
  // Handle process exit to ensure we check even if streams don't end properly
  child.on('close', function (code) {
    // Give streams a moment to finish, then force check
    setTimeout(function () {
      if (!callbackCalled) {
        stdoutEnded = true
        stderrEnded = true
