...
function execWithSpawn (argv, cwd, options, cb) {
  ...
  // For Windows, add Graphviz bin directory to PATH if using bundled Graphviz
  if (platform === 'win32' && dotPath) {
    // Check if this is a bundled Graphviz path (handle both / and \ separators)
    if (dotPath.includes('@node-plantuml-2/graphviz-') && (dotPath.includes('\\graphviz\\bin\\') || dotPath.includes('/graphviz/bin/'))) {
      var binDir = path.dirname(dotPath)
      var existingPath = env.PATH || ''
      // Add bin directory to PATH (Windows uses semicolon as separator)
      // Make sure binDir is normalized to Windows path format
      binDir = binDir.replace(/\//g, '\\')
      env.PATH = binDir + (existingPath ? ';' + existingPath : '')
    }
  }

  if (libPath || (platform === 'win32' && dotPath && dotPath.includes('@node-plantuml-2/graphviz-'))) {
