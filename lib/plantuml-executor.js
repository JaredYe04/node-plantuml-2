...
function execWithSpawn (argv, cwd, options, cb) {
  ...
  if (dotPath) {
    ...
    // Update argv with absolute path
    if (dotPathIndex >= 0) {
      // If dot path was found in argv, update it
      argv[dotPathIndex] = absoluteDotPath
    } else if (dotPathAutoDetected) {
      // CRITICAL FIX: If dot path was auto-detected, add it to argv
  ...
  // CRITICAL: On Windows, Graphviz dot.exe requires DLLs to be in PATH
  // Java subprocess will launch dot.exe, which needs to find its DLL dependencies
  if (platform === 'win32' && dotPath) {
    var binDir = path.dirname(dotPath)
    // Normalize to Windows path format (use backslashes)
    binDir = path.resolve(binDir).replace(/\//g, '\\')
    
    // On Windows, PATH might be Path (case-insensitive but Node.js preserves case)
    // We need to handle both PATH and Path keys
    var pathKey = 'PATH'
    ...
    // CRITICAL: Always add it at the beginning to ensure DLLs are found first
    // This is essential for bundled Graphviz to work correctly
    if (!alreadyInPath) {
      // Prepend bin directory to PATH
      env[pathKey] = binDir + (existingPath ? ';' + existingPath : '')
      // Also set PATH (lowercase) for compatibility with different Node.js versions
      env.PATH = env[pathKey]
      // Also set Path (mixed case) for maximum compatibility
      env.Path = env[pathKey]
      ...
      if (process.env.DEBUG_PLANTUML) {
        ...
        console.log('[DEBUG] PATH (first 500 chars):', env[pathKey].substring(0, 500))
      }
    } else if (process.env.DEBUG_PLANTUML) {
      console.log('[DEBUG] Graphviz bin directory already in PATH:', binDir)
    }
  }
