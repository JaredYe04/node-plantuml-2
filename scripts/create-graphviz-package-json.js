...
var OUTPUT_DIR = process.argv[5] || path.join(__dirname, '..', 'runtimes', '@node-plantuml-2', 'graphviz-' + PLATFORM + '-' + ARCH)

if (!PLATFORM || !ARCH || !VERSION) {
  console.error('Usage: node scripts/create-graphviz-package-json.js <platform> <arch> <version> [output-dir]')
  process.exit(1)
}
...
var packageName = '@node-plantuml-2/graphviz-' + PLATFORM + '-' + ARCH

var packageJson = {
  name: packageName,
  version: VERSION,
  description: 'Graphviz binaries for node-plantuml-2 on ' + PLATFORM + ' ' + ARCH,
  os: [PLATFORM],
  cpu: [ARCH],
  keywords: [
    'graphviz',
  ...
  repository: {
    ...
    directory: 'runtimes/@node-plantuml-2/graphviz-' + PLATFORM + '-' + ARCH
  },
  license: 'EPL-2.0',
